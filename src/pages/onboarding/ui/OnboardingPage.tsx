  ...
  return (
    <div className="flex min-h-svh flex-col items-center justify-center bg-gray-50 px-6 py-12">
      <div className="w-full max-w-sm">
        ...
        <p className="mb-8 text-sm text-gray-600">근무 패턴을 설정해주세요</p>

        {/* Step 1: 직종 선택 */}
        {step === 1 && (
          <div>
            <p className="mb-4 font-medium text-gray-900">직종을 선택해주세요</p>
            <div className="flex flex-col gap-3">
              {(Object.keys(JOB_TYPE_LABELS) as JobType[]).map((type) => (
                <button
                  ...
                  className="rounded-xl bg-white py-4 text-center text-lg font-semibold text-gray-900 shadow-sm ring-1 ring-gray-200 active:bg-gray-100"
                >
        ...
        {step === 2 && (
          <div>
            <button
              type="button"
              onClick={() => setStep(1)}
              className="mb-4 text-sm font-medium text-gray-700 active:text-gray-900"
            >
              ← 이전
            </button>
            <p className="mb-4 font-medium text-gray-900">소속 조를 선택해주세요</p>
        ...
        {step === 3 && jobType && (
          <div>
            ...
            <p className="mb-1 font-medium text-gray-900">기준일을 설정해주세요</p>
            <p className="mb-4 text-sm text-gray-600">특정 날짜에 어떤 근무였는지 알려주세요</p>

            <input
              type="date"
              value={baseDate}
              onChange={(e) => setBaseDate(e.target.value)}
              className="mb-4 w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-900 outline-none"
            />
